...
// Admin
def argocd_server_admin = "a474b72bf730811e8bd130234302828e-667024724.us-west-2.elb.amazonaws.com:443"
    ...
    node(ptNameVersion) {
        ...
        stage('Build') {
            withCredentials([usernamePassword(credentialsId: "artifactory-${serviceName}", passwordVariable: 'DOCKER_PASSWORD', usernameVariable: 'DOCKER_USERNAME')]) {
                container('cibuilder') {
                   sh "mkdir -p /go/src/github.com/argoproj"
                   sh "ln -sf \$(pwd) /go/src/github.com/argoproj/argo-cd"
                   sh "export DOCKER_HOST=127.0.0.1; docker login docker.artifactory.a.intuit.com -u $DOCKER_USERNAME -p $DOCKER_PASSWORD"
                   sh "export DOCKER_HOST=127.0.0.1; cd /go/src/github.com/argoproj/argo-cd; dep ensure && make controller-image server-image repo-server-image DOCKER_PUSH=true IMAGE_NAMESPACE=${registry}/${registry_path} IMAGE_TAG=latest"
                }
            }
        }
        // Handle the PR build
