...
def deploy_repo = "github.intuit.com/snikolaev1/cdp-deployment.git"
def tag = ""
...

{
    ...
    node(ptNameVersion) {
        ...
        stage('Build') {
                            container('cibuilder') {
                                sh ("mkdir -p /go/src/github.com/argoproj; ln -sf \$(pwd) /go/src/github.com/argoproj/argo-cd ; cd /go/src/github.com/argoproj/argo-cd; dep ensure && make controller-image server-image repo-server-image")
                            }
